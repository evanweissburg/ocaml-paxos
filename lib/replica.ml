...
let decide_instance ~instances ~seq ~v =
  ...
  Hashtbl.change instances seq ~f:(fun instance ->
      ...
      | Some instance -> (
          ...
          | Decided v' ->
              if String.(v <> v') then
                failwith "Instance already decided with different value!"
              else Some decided
...
let prepare_impl ~id ~min ~max ~instances ~done_estimates ()
    ((args, args_done_info) : Protocol.prepare_args * Protocol.done_info) =
  set_done ~done_estimates ~min ~instances ~id:args_done_info.sender
    args_done_info.seq;
  ...
  | Pending instance ->
      if args.n > !(instance.n_p) then (
        ...
        return
          ( Protocol.PrepareOk (args.n, !(instance.n_a), !(instance.v_a)),
            done_info ))
      else return (Protocol.PrepareReject, done_info)

let accept_impl ~id ~min ~max ~instances ~done_estimates ()
...
let learn_impl ~id ~min ~max ~instances ~done_estimates ()
...
let propose_impl ~id ~(replica_set : Common.replica_spec list) ~min ~max ~n
    ~instances ~done_estimates ~rpc_counter () (args : Protocol.propose_args) =
  let is_majority = Common.is_majority ~replica_set in
  let prepare_supported results =
    let is_decided =
      List.filter results ~f:(fun result ->
          match result with Protocol.PrepareDecided _ -> true | _ -> false)
    in
    ...
    | _ -> (
        ...
        let majority_value =
          ...
          in
          let rec loop (max_n, max_v, max_count) (cur_n, cur_v, cur_count) =
            function
            | [] ->
                if cur_count > max_count then (cur_v, cur_count)
                else (max_v, max_count)
            | (n, v) :: tl ->
                if n = cur_n then
                  loop (max_n, max_v, max_count)
                    (cur_n, cur_v, cur_count + 1)
                    tl
                else if cur_count > max_count then
        ...
        | None ->
            ...
            in
            if is_majority num_supporting then `Supported max_v
            else `NotSupported max_n)
  in
  let accept_supported results =
    let num_supporting =
      List.sum
        (module Int)
        results
        ~f:(function Protocol.AcceptOk _ -> 1 | Protocol.AcceptReject -> 0)
    in
  ...
  let apply_done_info results =
    let filter_ok results =
      List.fold results ~init:[] ~f:(fun acc result ->
          match result with Ok result -> result :: acc | Error _ -> acc)
    in
    let set_done_infos results =
      List.map results ~f:(fun (result, (done_info : Protocol.done_info)) ->
          set_done ~done_estimates ~min ~instances ~id:done_info.sender
            done_info.seq;
          result)
    in
  ...
  let rec propose_aux () =
    ...
    | `Pending n -> (
        ...
        let%bind results =
          broadcast_replicas ~rpc:Protocol.prepare_rpc ~local:local_prepare_impl
            ~args:prepare_args
        in
        ...
        | `WasDecided v ->
            ...
            let%bind _ =
              broadcast_replicas ~rpc:Protocol.learn_rpc ~local:local_learn_impl
                ~args:learn_args
            in
            return v
        | `NotSupported n' ->
        ...
        | `Supported v ->
            ...
            let%bind results =
              broadcast_replicas ~rpc:Protocol.accept_rpc
                ~local:local_accept_impl ~args:accept_args
            in
            if not (accept_supported results) then (
              ...
              let learn_args = (Protocol.{ seq = args.seq; v }, done_info) in
              let%bind _ =
                broadcast_replicas ~rpc:Protocol.learn_rpc
                  ~local:local_learn_impl ~args:learn_args
              in
              return v)
  in
  propose_aux ()
...
let implementations ~id ~replica_set ~n ~min ~max ~instances ~done_estimates
    ~rpc_counter =
  [
    Rpc.Rpc.implement Protocol.prepare_rpc
      (prepare_impl ~id ~min ~max ~instances ~done_estimates);
    Rpc.Rpc.implement Protocol.accept_rpc
      (accept_impl ~id ~min ~max ~instances ~done_estimates);
    Rpc.Rpc.implement Protocol.learn_rpc
      (learn_impl ~id ~min ~max ~instances ~done_estimates);
    Rpc.Rpc.implement Protocol.propose_rpc
      (propose_impl ~id ~min ~max ~replica_set ~n ~instances ~done_estimates
         ~rpc_counter);
  ]
...
let status ~min ~instances seq =
  if seq < !min then ForgottenStatus
  else
    match Hashtbl.find instances seq with
...
let start ~env ?(stop = Deferred.never ()) ~id
  ...
  let implementations =
    Rpc.Implementations.create_exn
      ~implementations:
        (implementations ~id ~replica_set ~n ~min ~max ~instances
           ~done_estimates ~rpc_counter)
      ~on_unknown_rpc:
        (`Call
          (fun _ ~rpc_tag ~version ->
            Log.Global.error "Unexpected RPC, tag %s, version %d" rpc_tag
              version;
            `Continue))
  in
  let%map server =
    Tcp.Server.create
      ~on_handler_error:
        (`Call (fun _ exn -> Log.Global.sexp [%sexp (exn : Exn.t)]))
