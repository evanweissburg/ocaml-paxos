  ...
  [
    {
      ...
      reliable;
      recv_disabled = num_recv_disabled >= 3;
    };
    {
      address = { host = "127.0.0.1"; port = 8766 };
      reliable;
      recv_disabled = num_recv_disabled >= 2;
    };
    {
      address = { host = "127.0.0.1"; port = 8767 };
      reliable;
      recv_disabled = num_recv_disabled >= 1;
    };
  ]
...
let replica_of_id ~replica_set ~id =
  ...
  | None ->
      failwith
        (Printf.sprintf "Replica id %d too large for n = %d" id
           (List.length replica_set))

let with_rpc_conn ~replica ~reliable f =
  try_with (fun () ->
      let rand_num = Random.int 100 in
      if ((not reliable) && rand_num < 10) || replica.recv_disabled then
        raise RPCFailure;
      let%map v =
        Tcp.with_connection
          (Tcp.Where_to_connect.of_host_and_port replica.address)
          ~timeout:(sec 1.) (fun _ r w ->
            match%bind
