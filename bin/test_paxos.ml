...
let count_decided ~(handles : Lib.Replica.handle list) ~seq ~allowed_vals =
  ...
  | hd :: tl ->
      ...
      then failwith "Commited value not allowed!"
        ...
        let filtered =
          List.filter tl ~f:(fun commit -> String.(commit <> hd))
        in
        if List.length filtered <> 0 then failwith "Not all values same!"
        else List.length committed

let rec wait_majority_decided ~(handles : Lib.Replica.handle list) ~seq
    ~allowed_vals =
  ...
  let (_ : unit list) =
    List.map commits ~f:(fun commit ->
        ...
        | Some v ->
            if
              not
                (List.exists messages ~f:(fun message -> String.(message = v)))
...
let test_forget ~stop () =
  ...
  let mins = List.map handles ~f:(fun handle -> handle.min ()) in
  if List.exists mins ~f:(fun min -> min <> 1) then
    failwith "Did not forget properly!";
...
let test_many_unreliable ~stop () =
  ...
  let%bind handles = start_replicas ~stop ~replica_set in
  let messages = [ "a"; "b"; "c" ] in
  Deferred.List.iter ?how:(Some `Parallel) (List.init 50 ~f:Fun.id)
    ~f:(fun seq ->
...
let test_efficient_1 ~stop () =
  ...
  let replica =
    match replica_set with [ _; _; two ] -> two | _ -> failwith "Impossible"
  in
  ...
  let other_replicas =
    match replica_set with _ :: tl -> tl | [] -> failwith "Impossible"
  in
  ...
  if count_rpcs handles <> 9 + 9 + 6 then
    failwith "Wrong number of RPCs issued!";
...
let test_efficient_2 ~stop () =
  ...
  Log.Global.printf
    "Proposer immediately decides after noticing decided acceptor...";
  let replica =
...
let run =
  ...
  let runnable_tests =
    List.fold tests ~init:(return ()) ~f:(fun acc test ->
        ...
        acc >>= test ~stop >>| Ivar.fill stop_ivar >>= Log.Global.flushed
        >>= fun () -> Clock.after (sec 0.1))
  in
  Log.Global.printf "Running %d tests:" (List.length tests);
  let%map _ =
    try runnable_tests
    with err -> return (Log.Global.printf "%s" (Exn.to_string err))
  in
