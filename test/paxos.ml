...
let%expect_test "can provide consistent proposal results to replicas" = 
  ...
  match commits with 
    ...
    | hd::tl -> 
      let filtered = List.filter tl ~f:(fun commit -> String.(commit <> hd)) in 
      if List.length filtered <> 0 then failwith "Replicas received inconsistent commit results" else 
      ()

let%expect_test "can commit a value" = 
  let stop_ivar = Ivar.create () in
  let stop = Ivar.read stop_ivar in
  let replica_set = Lib.Common.default_replica_set ~reliable:false () in
...
let%expect_test "can commit a value with a peer that cannot recv" = 
  let stop_ivar = Ivar.create () in
  let stop = Ivar.read stop_ivar in
